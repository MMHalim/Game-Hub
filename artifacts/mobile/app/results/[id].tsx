...
export default function ResultsScreen() {
  ...
  useEffect(() => {
    if (activeSession?.id === id) setSession(activeSession);
    else if (id) loadSession(id).then(setSession);
  }, [id]);
  ...
  if (!session) {
    return (
      <View style={[styles.loading, { backgroundColor: colors.background }]}>
        <Text style={{ color: colors.foreground }}>Loading results...</Text>
      </View>
    );
  }
  ...
  const handlePlayAgain = () => { resetGame(); router.replace("/(tabs)"); };
  ...
  return (
    <LinearGradient colors={colors.gradientBg} style={styles.gradient}>
      ...
      >
        {/* Winner Spotlight */}
        <Animated.View style={[styles.winnerSection, {
          opacity: crownAnim,
          ...
          <Text style={[styles.winnerCeleb, { color: colors.mutedForeground }]}>
            {getCelebrityById(winner.celebrityId)?.emoji}{" "}
            {getCelebrityById(winner.celebrityId)?.name}
          </Text>
          <View style={[styles.winnerScoreBadge, { backgroundColor: colors.gold + "22" }]}>
            <Feather name="mic" size={16} color={colors.gold} />
            <Text style={[styles.winnerScore, { color: colors.gold }]}>{Math.round(winner.score)} pts</Text>
          </View>
        </Animated.View>
            ...
            {[sorted[1], sorted[0], sorted[2]].map((player, idx) => {
              ...
              return (
                <View key={player.id} style={styles.podiumItem}>
                  <Text style={[styles.podiumName, { color: colors.foreground }]} numberOfLines={1}>{player.name}</Text>
                  <Text style={[styles.podiumScore, { color: colors.gold }]}>{Math.round(player.score)}</Text>
                  <LinearGradient colors={podiumColors[idx]} style={[styles.podiumBar, { height }]}>
        ...
        {/* Scoreboard */}
        <Text style={[styles.scoreboardTitle, { color: colors.mutedForeground }]}>Full Scoreboard</Text>
        {sorted.map((player, i) => (
          <Animated.View key={player.id} style={{
            opacity: staggerAnims[i],
            transform: [{ translateX: staggerAnims[i].interpolate({ inputRange: [0, 1], outputRange: [40, 0] }) }],
          }}>
            <PlayerCard
              name={player.name}
        ...
        <View style={styles.actions}>
          <Pressable style={({ pressed }) => [styles.playAgainBtn, { opacity: pressed ? 0.85 : 1 }]} onPress={handlePlayAgain}>
            <LinearGradient
          ...
          <Pressable
            style={({ pressed }) => [styles.homeBtn, { borderColor: colors.border, opacity: pressed ? 0.8 : 1 }]}
            onPress={() => { resetGame(); router.replace("/(tabs)"); }}
          >
            <Feather name="home" size={18} color={colors.mutedForeground} />
            <Text style={[styles.homeText, { color: colors.mutedForeground }]}>Home</Text>
          </Pressable>
        </View>
      </ScrollView>
    </LinearGradient>
...
const styles = StyleSheet.create({
  ...
  winnerLabel: { fontSize: 11, fontWeight: "700", fontFamily: "Inter_700Bold", letterSpacing: 2 },
  winnerName: { fontSize: 38, fontWeight: "800", fontFamily: "Inter_700Bold" },
  winnerCeleb: { fontSize: 16, fontFamily: "Inter_400Regular" },
  winnerScoreBadge: { flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 16, paddingVertical: 7, borderRadius: 20, marginTop: 4 },
  winnerScore: { fontSize: 18, fontWeight: "700", fontFamily: "Inter_700Bold" },
  ...
  podiumName: { fontSize: 11, fontFamily: "Inter_500Medium", marginBottom: 4, textAlign: "center" },
  podiumScore: { fontSize: 13, fontWeight: "700", fontFamily: "Inter_700Bold", marginBottom: 4 },
  ...
  scoreboardTitle: { fontSize: 14, fontWeight: "700", fontFamily: "Inter_700Bold", letterSpacing: 0.8, textTransform: "uppercase" },
